  ...
  <v-flex>
    <v-col cols="12" md="10" offset="1">
      ...
      >
        <template v-slot:top>
          <v-toolbar flat>
            <v-toolbar-title>Seguros</v-toolbar-title>
                ...
                <v-card-text>
                  <v-container>
                    ...
                    <v-row>
                      ...
                      <v-col cols="12" sm="6" md="6">
                        <v-text-field
                          ...
                          label="Nombre del seguro ingles"
                        ></v-text-field>
                      </v-col>
                    </v-row>
                    <v-row justify="center">
                      ...
                      <v-col cols="12" sm="6" md="6">
                        <v-textarea
                          v-model="editedItem.descriptionUs"
            ...
            <v-dialog v-model="dialogDelete" max-width="500px">
              <v-card>
                <v-card-title class="text-h5"
                  >Seguro que desea eliminar este seguro?</v-card-title
                >
                <v-card-actions>
                  <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Nuevo seguro" : "Editar Seguro";
    },
  },
    ...
    async deleteItemConfirm() {
      try {
        this.data = await this.$apollo.mutate({
          mutation: DELETE_INSURANCES,
          variables: {
            insuranceId: this.editedItem.rowId,
          },
        });
      } catch (err) {
    ...
    async save() {
      if (this.editedIndex > -1) {
        var data = Object.assign(
          this.insurances[this.editedIndex],
          this.editedItem
        );

        try {
          this.data = await this.$apollo.mutate({
            mutation: UPDATE_INSURANCES,
            variables: {
              insuranceId: data.rowId,
              companyId: this.$companie_id,
              name: data.name,
              amount: data.amount,
              nameUs: data.nameUs,
              description: data.description,
              descriptionUs: data.descriptionUs,
        ...
        try {
          this.data = await this.$apollo.mutate({
            ...
            variables: {
              ...
              descriptionUs: this.editedItem.descriptionUs,
              currencyId: this.selectCurrencie,
            },
          });
        } catch (err) {

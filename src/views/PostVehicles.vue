<template>
  <v-flex>
    ...
    <v-row justify="center">
      <v-col cols="12" md="12">
        <v-stepper v-if="loadingData" class="elevation-0" v-model="e1">
          <v-stepper-items>
            ...
            <v-stepper-content step="11">
              ...
              >
                <v-row justify="center" no-gutters>
                  <v-col cols="12">
                    <v-row justify="center" no-gutters>
                      <span class="my-application2"> Vista previa </span>
                    </v-row>
                    <v-row justify="center">
                      <v-card-title
...
import { POST_VEHICLES } from "../gql/mutation";
import { GET_REQUERIMENT_VEHICLES } from "../gql/queries";
export default {
  data() {
    return {
      loadingData: false,
  ...
  methods: {
    ...
    deleteItem(item, index) {
      ...
      // this.onDrop({ id:index, path: item });
    },
    ...
    async getData() {
      ...
      var vehicles = await this.$apollo.query({
        query: GET_REQUERIMENT_VEHICLES,
      ...
      this.modelVehicles = vehicles.data.vehicleModels.nodes;
      ...
      try {
        this.data = await this.$apollo.mutate({
          ...
          variables: {
            ...
            owner: JSON.parse(localStorage.getItem("user")).rowId,
            ...
            isDeluxe: this.selectDeluxe ? true : false,
          },
        });

        this.$router.push(
          "/vehicles/" + this.data.data.publishAVehicle.vehicle.rowId
        );
      } catch (err) {
        this.snackbar = true;
        ...
        json.forEach((j) => {
          ...
          if (palabra.includes(value.toLowerCase())) {
            resultados.push({
              rowId: j.rowId,
              name: j.name,
              vehicleBrandId: j.vehicleBrandId,
            });
          }
        });
        return resultados;
    ...
    addTipVehicles(item) {
      ...
      // if (this.selectedTipVehicles.rowId == item.rowId) {
      ...
      // } else {
      //   this.selectedTipVehicles = item;
      // }
    },
    nextTipVehicles() {
      if (this.selectedTipVehicles.length == 0) {
        this.snackbar = true;
    ...
    searchItemModel() {
      ...
      for (var items of this.modelVehicles) {
        if (items.vehicleBrandId == this.selectedBranVehicles.rowId) {
